...
import { RefreshCw, AlertTriangle, Activity, UserCheck, Loader2 } from 'lucide-react'
                    ...
                    {disabled.map((w, i) => (
                        <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-2">
                            <div className="min-w-0">
                                <div className="text-xs font-medium truncate">{w.account_id}</div>
                                <div className="text-[10px] text-muted-foreground mt-0.5">
                                    {t('accountManager.failCount', { count: w.consecutive_fails })}
                                    {w.last_failure_at && ` · ${t('accountManager.lastFail')}: ${new Date(w.last_failure_at).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}`}
                                </div>
                            </div>
                            <button
                    ...
                    {degraded.map((w, i) => (
                        <div key={i} className="px-4 py-2 flex items-center justify-between gap-2">
                            <div className="min-w-0">
                                <div className="text-xs font-medium truncate">{w.account_id}</div>
                                <div className="text-[10px] text-muted-foreground mt-0.5">
                                    <span className="text-amber-500 font-medium">{w.current_weight}/{w.max_weight}</span>
